...
static void ssd_init_write_pointer(struct ssd *ssd)
{
    ...
    ssd->trans_wp.wpl = malloc(sizeof(struct wp_lines));
    ...
    ssd->line2write_pointer = malloc(sizeof(struct write_pointer *) * ssd->sp.tt_lines);
...
static void advance_line_write_pointer (struct ssd *ssd, struct write_pointer *wpp) {
    ...
    if (wpp->ch == spp->nchs) {
        ...
        if (wpp->lun == spp->luns_per_ch) {
            ...
            wpp->pg++;
            // printf("wpp pg vs pgs_per_blk: %d %d\n", wpp->pg, spp->pgs_per_blk);
...
static void ssd_init_maptbl(struct ssd *ssd)
{
    ...
    ssd->valid_lines = malloc(sizeof(uint8_t) * spp->tt_lines);
...
static void insert_entry_to_cmt(struct ssd *ssd, uint64_t lpn, uint64_t ppn, int pos, bool prefetch, uint64_t next_avail_time)
{
    ...
    if (tpnode == NULL || tpnode->tvpn != tvpn) {
        ...
        tpnode = malloc(sizeof(struct TPnode));
        tpnode->tvpn = tvpn;
...
uint64_t* unique_and_sort(uint64_t* lpn, uint64_t num, uint64_t* new_num) {
    if (num == 0) {
        *new_num = 0;
        return NULL;
    }

    qsort(lpn, num, sizeof(uint64_t), compare);

    uint64_t* unique_lpn = (uint64_t*)malloc(num * sizeof(uint64_t));
    uint64_t j = 0;
    unique_lpn[j++] = lpn[0];
